...
export default function Grainient({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ webgl: 2, alpha: true, antialias: false, dpr: Math.min(window.devicePixelRatio || 1, 2) } as any);
    const gl = renderer.gl as unknown as WebGL2RenderingContext & { canvas: HTMLCanvasElement };
    ...
    const setSize = () => {
      ...
      const res = (program as any).uniforms.iResolution.value as Float32Array;
    ...
    const loop = (t: number) => {
      (program as any).uniforms.iTime.value = (t - t0) * 0.001;
      renderer.render({ scene: mesh });
