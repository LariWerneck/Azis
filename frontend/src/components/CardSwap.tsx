...
const makeSlot = (i: number, distX: number, distY: number, total: number): SlotConfig => ({
  x: i * distX,
  y: -i * distY,
  z: -i * distX * 1.5,
  zIndex: total - i,
...
const placeNow = (el: HTMLElement, slot: SlotConfig, skew: number) =>
  gsap.set(el, {
    x: slot.x,
    y: slot.y,
    z: slot.z,
...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  const config = EASING_CONFIG[easing];
