...
export default function Feed() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (activeTab === "team") return feedEvents.filter((event) => event.isTeam === true);
    return feedEvents;
  }, [activeTab, feedEvents]);

  const fetchFeedData = async (pageToLoad = 1) => {
    try {
      const [feedResponse, leaderboardResponse, badgesResponse, onlineResponse] = await Promise.allSettled([
        fetch(getApiUrl(`/api/feed?limit=10&page=${pageToLoad}`), { headers: getAuthHeaders(), credentials: 'include' }),
        fetch(getApiUrl("/api/feed/leaderboard"), { headers: getAuthHeaders(), credentials: 'include' }),
        fetch(getApiUrl("/api/badges"), { headers: getAuthHeaders(), credentials: 'include' }),
        fetch(getApiUrl("/api/feed/online"), { headers: getAuthHeaders(), credentials: 'include' }),
      ]);
      ...
      if (feedResponse.status === "fulfilled" && feedResponse.value.ok) {
        ...
        if (Array.isArray(raw)) {
          const parsed = raw.map((item: any, index: number) => ({
            ...
            userId: item.userId?.toString() ?? item.user_id?.toString() ?? `u-${index}`,
            userName: item.userName ?? item.username ?? item.user?.name ?? item.name ?? "Usuário",
            userAvatar: item.userAvatar ?? item.user_avatar ?? undefined,
            userLevel: Number(item.userLevel ?? item.user_level ?? 1),
            type: item.type ?? "task",
            description: item.description ?? item.text ?? "Concluiu uma ação.",
            stampEmoji: item.stampEmoji ?? item.stamp_emoji ?? "🏅",
            points: Number(item.points ?? item.value ?? 0),
            pointsType: item.pointsType ?? item.points_type ?? item.pointstype ?? "gold",
            isNew: Boolean(item.isNew ?? item.is_new ?? item.unread ?? false),
            ...
            reactions: Array.isArray(item.reactions)
              ...
              : [],
            isTeam: Boolean(item.isTeam ?? item.is_team ?? true),
          })) as FeedEvent[];
        ...
        if (Array.isArray(raw)) {
          setLeaderboard(
            raw.slice(0, 5).map((item: any, index: number) => ({
              ...
              userId: item.userId?.toString() ?? item.user_id?.toString() ?? `lb-${index}`,
              name: item.name ?? item.fullName ?? "Usuário",
              avatar: item.avatar ?? undefined,
              lastBadgeTitle: item.lastBadgeTitle ?? item.last_badge_title ?? "Selo recente",
              score: Number(item.score ?? item.total_points ?? 0),
            })),
          );
        }
      }
        ...
        if (Array.isArray(raw)) {
          setOnlineUsers(
            raw.slice(0, 20).map((item: any, index: number) => ({
              userId: item.userId?.toString() ?? item.user_id?.toString() ?? `online-${index}`,
              name: item.name ?? item.fullName ?? "Usuário",
              avatar: item.avatar ?? undefined,
            })),
          );
        }
        setOnlineCount(Number(payload?.count ?? payload?.onlineCount ?? payload?.count ?? 0));
      }
    } catch (error) {
            ...
            {FEED_TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`relative rounded-[9px] px-4 py-2 text-[13px] font-[700] font-[Nunito] transition ${isActive
          ...
          <div className="space-y-3">
            {filteredEvents.map((event, index) => (
              ...
              >
                ...
                <div className={`absolute left-0 top-4 bottom-4 w-1.5 rounded-r-[3px] ${event.pointsType === "gold"
                ...
                <div className="flex items-start gap-4 ml-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-[color:var(--feed-text3)]">
                      ...
                      <span className={`inline-flex rounded-full px-3 py-1 text-[11px] font-[700] ${event.pointsType === "gold"
                    ...
                    <div className="mt-4 flex flex-wrap gap-2 border-t border-[color:var(--feed-divider)] pt-3 text-[12px] text-[color:var(--feed-text2)]">
                      {event.reactions.map((reaction, reactionIndex) => (
                        <button
                          key={`${reaction.emoji ?? 'reaction'}-${reactionIndex}`}
                          type="button"
            ...
            {filteredEvents.length === 0 && !isLoadingMore && (
              <div className="rounded-[18px] border border-[color:var(--feed-card-border)] bg-[color:var(--feed-card)] p-8 text-center text-[color:var(--feed-text2)]">
                ...
                <div className="text-[16px] font-[700] text-[color:var(--feed-text)] mb-2">Nenhum evento encontrado</div>
                <div className="text-[14px] text-[color:var(--feed-text3)]">Seja o primeiro a completar uma tarefa ou desbloquear um selo!</div>
              </div>
            )}
            {isLoadingMore && (
            ...
            <ParticleCard className="rounded-[18px] border border-[color:var(--feed-card-border)] bg-[color:var(--feed-card)] p-5" clickEffect={true} enableStars={true}>
              ...
              <div className="space-y-2">
                {leaderboard.map((player) => (
                  <button
                    key={`${player.userId ?? 'lb'}-${player.rank}`}
                    type="button"
                    className="flex w-full items-center gap-3 rounded-[12px] px-3 py-2 transition hover:bg-[color:var(--feed-bg2)]"
                  >
                    <span className={`inline-flex h-8 w-8 items-center justify-center rounded-[10px] text-[12px] font-[700] ${player.rank === 1 ? "bg-[color:var(--feed-yellow)] text-[#1a1200]" : player.rank === 2 ? "text-[color:var(--feed-text3)]" : player.rank === 3 ? "text-[#cd7f32]" : "text-[color:var(--feed-text2)]"
                      }`}>
                      {player.rank}
                    </span>
            ...
            <ParticleCard className="rounded-[18px] border border-[color:var(--feed-card-border)] bg-[color:var(--feed-card)] p-5" clickEffect={true} enableStars={true}>
              ...
              <div className="space-y-2">
                {onlineUsers.slice(0, 4).map((user, index) => (
                  <div key={`${user.userId ?? 'online'}-${index}`} className="relative flex items-center gap-2 py-1 text-[12px] font-[700] text-[color:var(--feed-text2)]">
